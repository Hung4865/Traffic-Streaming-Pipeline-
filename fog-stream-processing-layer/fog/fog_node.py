...
CAMERA_URL_PARAMS = os.environ.get("CAMERA_URL_PARAMS", "")
...
def initialize_fog_node(mac_address):
    global camera_id, stop_capture_event
    ...
    camera_url_params = CAMERA_URL_PARAMS.strip()

    if not camera_url:
        logging.error("CAMERA_URL is empty. Aborting.")
        return

    full_camera_url = camera_url if not camera_url_params else f"{camera_url}?{camera_url_params}"

    logging.info("Using static camera configuration. Camera ID: %s", camera_id)
